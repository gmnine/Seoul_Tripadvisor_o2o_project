...
def update_file_processed_review(processed_review):
    file_clean_review_csv.write(processed_review[0] + processed_review[1])
...
#------------------- 라인별 텍스트 출력 --------------------
for i in range(0, 5):#len(total_lines)):
    print("line",str(i),"=",total_lines[i])
    #---------------------- 문장 분리 -------------------------
    sentence_tokenized_text = sentence_strip(total_lines[i])
